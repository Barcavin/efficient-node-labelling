...
def read_musae_one(raw_dir, feat_dim):
    # three files to read. xxx-edges.txt, xxx.json, xxx-target.csv
    # list files:
    for file in os.listdir(raw_dir):
        if file.endswith('edges.csv'):
            ...
            target_file = file
    import pandas as pd
    edge_index = pd.read_csv(os.path.join(raw_dir, edge_file), sep=',', header=None, skiprows=1,
                             dtype=np.int64)
    edge_index = torch.from_numpy(edge_index.values).t()
    ...
    num_nodes = max(len(json_dict), edge_index.max().item() + 1)
    edge_index = coalesce(edge_index, num_nodes=num_nodes)
    # convert to one-hot encoding
    feat_dim = max([v1 for v in json_dict.values() for v1 in v]) + 1 if feat_dim is None else feat_dim
    x = torch.zeros(num_nodes, feat_dim, dtype=torch.float)
    for idx, feature in json_dict.items():
        idx = int(idx)
        x[idx, feature] = 1
    data = Data(x=x, edge_index=edge_index, num_nodes=num_nodes)
    return data

def read_musae(raw_dir):
    if "twitch" in raw_dir:
        data_list = []
        for file in os.listdir(raw_dir):
            file_path = os.path.join(raw_dir, file)
            # if file is a dir
            if os.path.isdir(file_path):
                data_list.append(read_musae_one(file_path, 3170))
    else:
        data_list = [read_musae_one(raw_dir, None)]
    return data_list
...
class SNAPDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        elif self.name[:5] == 'musae':
            data_list = read_musae(raw_dir)
        else:
...
if __name__ == '__main__':
    root = "data/"
    dataset = SNAPDataset(root, 'musae-twitch')
    dataset = SNAPDataset(root, 'musae-facebook')
    dataset = SNAPDataset(root, 'musae-github')
    # read_musae_one('tmp/twitch/RU')
